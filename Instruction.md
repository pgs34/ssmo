...
SSML이 주장하는 포인트를 실험으로 “증명”하려면 아래 4개를 각각 때려야 합니다.
	...
	3.	Negative transfer 억제: 약한 peer가 있을 때 DML은 강한 모델을 망치기 쉽고, SSML은 덜 망치거나 보호
...
1.3 비교할 방법(알고리즘)
	...
	•	DML: imitation 항상 ON에 가까운 dense mutual distillation
	•	분류에서는 imitation = KL(softmax)
	•	SSML: per-sample CE(또는 NLL) 비교 기반 selective imitation
...
1.5 기대 결과(무슨 그림/표가 나와야 “승리”인지)
	...
	•	DML은 강한 모델 성능을 깎는 케이스가 반드시 나올 확률이 높음(negative transfer)
	•	SSML은 그 degradation을 거의 막거나 크게 줄이는 형태가 나와야 함
	•	dynamics:
	•	SSML의 conflict rate가 DML보다 낮아야 함(특히 hetero pair에서)
...
2.3 비교할 방법
	•	Independent
	•	DML: operator에서는 dense imitation = L2 everywhere에 해당
	•	SSML: grid-point wise selective mask
...
2.5 기대 결과(“SSML이 필요한 이유”가 드러나는 포인트)
	...
	1.	heterogeneous pair에서 DML이 불안정/성능 저하가 나오기 쉬움
...
Time-series는 classification/operator와 동등 중요도로 수행합니다.
...
3.5 기대 결과
	...
	•	DML은 한 모델이 특정 horizon에서 약하면 그 약점이 전파되어 역효과가 나올 수 있음 → SSML의 차별점

⸻

4) 4.4 Additional experiments를 “실행 단위”로 쪼갠 체크리스트
...
기대:
	•	warm-up/τ가 mis-selection을 줄여서 안정화
...
기대:
	•	SSML이 DML보다 conflict rate가 낮고(특히 hetero), 곡선이 더 안정적

⸻

5) “반드시 돌려야 하는 동등 중요도 실험 세트”

3개 태스크를 동일 우선순위로 수행합니다.
	...
	4.	Dynamics 로그(conflict/cosine/||g_im||/||g_sup||/mask ratio)는 1)~3) 모두에서 공통으로 수집
...
원하시면 다음으로는, 위 설계를 바탕으로 **각 태스크별 “실행 계획표”**를 만들겠습니다. 예를 들면:
	...
	•	(methods) Indep / DML / SSML
	•	(settings) clean + CIFAR-100-C + label-noise 0.4
	•	(expected) DML에서 strong drop, SSML에서 drop 억제
