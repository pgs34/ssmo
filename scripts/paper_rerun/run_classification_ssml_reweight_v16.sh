...
LOG_DIR="${LOG_DIR:-results/logs/classification_ssml_reweight_v16}"
mkdir -p "$LOG_DIR"
...
EPOCHS="${EPOCHS:-100}"
OUTPUT_ROOT="${OUTPUT_ROOT:-results/classification_ssml_reweight_v16}"
DOWNLOAD="${DOWNLOAD:-1}"
...
SSML_GATE_SCORE_MODE="${SSML_GATE_SCORE_MODE:-useful_hard_sample}"
...
CASE_SPECS="${CASE_SPECS:-uh_pb30_aw5e4:0.30:0.04:0.010:0.030:0.40:4:6.0:0.0005 uh_pb40_aw1e3:0.40:0.03:0.015:0.040:0.45:6:8.0:0.0010}"
...
run_job() {
  ...
  shift
  run_logged_job \
    "classification_reweight_v16/$label" \
    "$LOG_DIR/$label.log" \
    env \
...
echo "[classification_ssml_reweight_v16] output_root=$OUTPUT_ROOT"
echo "[classification_ssml_reweight_v16] gpu=$GPU datasets=$DATASETS seeds=$SEEDS"
echo "[classification_ssml_reweight_v16] model_pairs=$MODEL_PAIRS"
echo "[classification_ssml_reweight_v16] warmup=$WARMUP_EPOCHS decay=${IMITATION_DECAY_START_EPOCH}->${IMITATION_DECAY_END_EPOCH} min_scale=$IMITATION_DECAY_MIN_SCALE"
echo "[classification_ssml_reweight_v16] freeze_peer=$SSML_FREEZE_PEER student_only=$SSML_STUDENT_ONLY worse_only=$SSML_WORSE_ONLY_UPDATE"
echo "[classification_ssml_reweight_v16] gate=$SSML_GATE_SCORE_MODE transform=$SSML_SCORE_TRANSFORM guidance=$SSML_GUIDANCE_MODE"
echo "[classification_ssml_reweight_v16] case_specs=$CASE_SPECS"
