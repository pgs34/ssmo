...
mkdir -p "$LOCK_ROOT"
exec 9>"$LOCK_ROOT/launch_worker1_cifar100_strict128_dual_v2_wait_gpu0.lock"
flock -n 9 || {
  echo "[worker1_cifar100_strict128_dual_v2_wait_gpu0] launcher already running"
  exit 0
}

LOG_DIR="${LOG_DIR:-results/logs/classification_cifar100_strict128_dual_v2/worker1_gpu0}"
mkdir -p "$LOG_DIR"
...
CASE_SPEC="${CASE_SPEC:-uh_cons_ag55:useful_hard_sample_confident:0.24:0.04:0.012:0.020:0.35:0.020:4:6.0:0.0004:0.85:0.80:2:0.50:0.03:0.72:0.92:0.03:4:18:55:0.30:0.55}"
...
while ! gpu_is_available; do
  echo "[$(timestamp)] [worker1_cifar100_strict128_dual_v2_wait_gpu0] waiting for gpu=$TARGET_GPU util<=$FREE_GPU_MAX_UTIL mem<=$FREE_GPU_MAX_MEM_MIB MiB"
...
run_logged_job \
  ...
  env \
    ...
    OUTPUT_ROOT="results/classification_cifar100_strict128_dual_v2/smoke_worker1_gpu0" \
    LOG_DIR="results/logs/classification_cifar100_strict128_dual_v2/smoke_worker1_gpu0/cases" \
...
run_logged_job \
  ...
  env \
    ...
    BATCH_SIZE="${BATCH_SIZE_2080TI:-128}" \
    NUM_WORKERS="${NUM_WORKERS_2080TI:-4}" \
    OUTPUT_ROOT="results/classification_cifar100_dual_peer_consensus_strict128_v2/worker1_gpu0" \
    LOG_DIR="results/logs/classification_cifar100_dual_peer_consensus_strict128_v2/worker1_gpu0/cases" \
...
echo "[worker1_cifar100_strict128_dual_v2_wait_gpu0] finished"
