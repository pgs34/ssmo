...
mkdir -p "$LOCK_ROOT"
exec 9>"$LOCK_ROOT/launch_worker2_etth1_snapshot_then_delta_v1.lock"
flock -n 9 || {
  echo "[worker2_etth1_snapshot_then_delta_v1] launcher already running"
  exit 0
}
...
echo "[worker2_etth1_snapshot_then_delta_v1] all jobs finished"
