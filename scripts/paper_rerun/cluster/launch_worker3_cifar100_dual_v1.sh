...
mkdir -p "$LOCK_ROOT"
exec 9>"$LOCK_ROOT/launch_worker3_cifar100_dual_v1.lock"
flock -n 9 || {
  echo "[worker3_cifar100_dual_v1] launcher already running"
  exit 0
}
...
run_logged_job \
  "worker3/cifar100_dual_peer_consensus_v1_gpu${GPU}" \
  ...
  env \
    ...
    OUTPUT_ROOT="results/classification_cifar100_dual_peer_consensus_v1/worker3_gpu0" \
    LOG_DIR="results/logs/classification_cifar100_dual_peer_consensus_v1/worker3_gpu0/cases" \
...
echo "[worker3_cifar100_dual_v1] finished"
