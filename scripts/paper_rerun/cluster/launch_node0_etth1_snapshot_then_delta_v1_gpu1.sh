...
mkdir -p "$LOCK_ROOT"
exec 9>"$LOCK_ROOT/launch_node0_etth1_snapshot_then_delta_v1_gpu1.lock"
flock -n 9 || {
  echo "[node0_etth1_snapshot_then_delta_v1_gpu1] launcher already running"
  exit 0
}
...
echo "[node0_etth1_snapshot_then_delta_v1_gpu1] all jobs finished"
