...
mkdir -p "$LOCK_ROOT"
exec 9>"$LOCK_ROOT/launch_worker1_etth1_delta_fusion_v1_wait_gpu1.lock"
flock -n 9 || {
  echo "[worker1_etth1_delta_fusion_v1_wait_gpu1] launcher already running"
  exit 0
}
...
while ! gpu_is_available; do
  echo "[$(timestamp)] [worker1_etth1_delta_fusion_v1_wait_gpu1] waiting for gpu=$TARGET_GPU util<=$FREE_GPU_MAX_UTIL mem<=$FREE_GPU_MAX_MEM_MIB MiB"
  sleep "$POLL_INTERVAL_SEC"
done

run_logged_job \
  "worker1/etth1_delta_fusion_v1_gpu${TARGET_GPU}" \
  ...
  env \
    ...
    OUTPUT_ROOT="results/time_series_etth1_delta_fusion_v1/worker1_gpu1" \
    LOG_DIR="results/logs/time_series_etth1_delta_fusion_v1/worker1_gpu1/cases" \
...
echo "[worker1_etth1_delta_fusion_v1_wait_gpu1] finished"
