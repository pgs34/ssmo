...
LOG_DIR="${LOG_DIR:-results/logs/instruction_matrix_v1/worker2}"
mkdir -p "$LOG_DIR"
...
if [[ "$ENABLE_PLOT_WATCHER" == "1" ]]; then
  LOGFILE="$LOG_DIR/time_series_plots.log" INTERVAL_SEC="$PLOT_INTERVAL_SEC" LABEL="time_series" \
    bash scripts/paper_rerun/watch_summary_plots.sh \
    "results/instruction_matrix_v1/time_series" \
    "$PLOT_ROOT/time_series" &
  PLOT_WATCH_PID="$!"
  echo "[worker2_instruction_matrix_v1] plot watcher pid=$PLOT_WATCH_PID"
fi
...
bash scripts/paper_rerun/refresh_summary_plots.sh \
  "results/instruction_matrix_v1/time_series" \
  "$PLOT_ROOT/time_series" \
  >>"$LOG_DIR/time_series_plots.log" 2>&1 || true

echo "[worker2_instruction_matrix_v1] job finished"
