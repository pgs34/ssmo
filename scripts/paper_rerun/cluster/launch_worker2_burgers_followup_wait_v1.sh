...
mkdir -p "$LOCK_ROOT"
exec 9>"$LOCK_ROOT/launch_worker2_burgers_followup_wait_v1.lock"
flock -n 9 || {
  echo "[worker2_burgers_followup_wait_v1] launcher already running"
  exit 0
}

LOG_DIR="${LOG_DIR:-results/logs/operator_burgers_followup_v1/worker2}"
mkdir -p "$LOG_DIR"
...
while ! gpu_is_available; do
  echo "[$(timestamp)] [worker2_burgers_followup_wait_v1] waiting for gpu=$TARGET_GPU util<=$FREE_GPU_MAX_UTIL mem<=$FREE_GPU_MAX_MEM_MIB MiB"
  sleep "$POLL_INTERVAL_SEC"
done

echo "[$(timestamp)] [worker2_burgers_followup_wait_v1] selected gpu=$TARGET_GPU"

run_logged_job \
  "worker2/operator_burgers_followup_v1" \
  ...
  env \
    ...
    bash scripts/paper_rerun/run_operator_burgers_followup_v1.sh

echo "[worker2_burgers_followup_wait_v1] job finished"
