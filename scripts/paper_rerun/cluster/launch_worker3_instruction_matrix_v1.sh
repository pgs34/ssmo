...
LOG_DIR="${LOG_DIR:-results/logs/instruction_matrix_v1/worker3}"
mkdir -p "$LOG_DIR"
...
if [[ "$ENABLE_PLOT_WATCHER" == "1" ]]; then
  LOGFILE="$LOG_DIR/operator_plots.log" INTERVAL_SEC="$PLOT_INTERVAL_SEC" LABEL="operator" \
    bash scripts/paper_rerun/watch_summary_plots.sh \
    "results/instruction_matrix_v1/operator" \
    "$PLOT_ROOT/operator" &
  PLOT_WATCH_PID="$!"
  echo "[worker3_instruction_matrix_v1] plot watcher pid=$PLOT_WATCH_PID"
fi
...
bash scripts/paper_rerun/refresh_summary_plots.sh \
  "results/instruction_matrix_v1/operator" \
  "$PLOT_ROOT/operator" \
  >>"$LOG_DIR/operator_plots.log" 2>&1 || true

echo "[worker3_instruction_matrix_v1] job finished"
