...
LOG_DIR="${LOG_DIR:-results/logs/classification_ssml_reweight_v20}"
mkdir -p "$LOG_DIR"
...
EPOCHS="${EPOCHS:-100}"
OUTPUT_ROOT="${OUTPUT_ROOT:-results/classification_ssml_reweight_cifar100_v20}"
DOWNLOAD="${DOWNLOAD:-1}"
...
CASE_SPECS="${CASE_SPECS:-safe_pb20_gap08:0.20:0.05:0.010:0.015:0.40:4:6.0:0.0005:0.45:0.08 safe_pb25_gap10:0.25:0.05:0.012:0.020:0.45:4:6.0:0.0005:0.42:0.10}"
...
if [[ -z "${PEER_INIT_CHECKPOINT_TEMPLATE:-}" ]]; then
  PEER_INIT_CHECKPOINT_TEMPLATE='results/classification_ssml_reweight_cifar100_v17_alt/conf_pb25_aw5e4/classification/{dataset}/resnet34_gelu_ssml_{classification_imitation_loss}_seed{seed}/model.pt'
fi
...
run_job() {
  ...
  shift
  run_logged_job \
    "classification_reweight_v20/$label" \
    "$LOG_DIR/$label.log" \
    env \
...
echo "[classification_ssml_reweight_v20] output_root=$OUTPUT_ROOT"
echo "[classification_ssml_reweight_v20] gpu=$GPU datasets=$DATASETS seeds=$SEEDS"
echo "[classification_ssml_reweight_v20] model_pairs=$MODEL_PAIRS"
echo "[classification_ssml_reweight_v20] case_specs=$CASE_SPECS"

for spec in $CASE_SPECS; do
  IFS=':' read -r label topk_ratio alpha lambda_imitation margin prob_threshold per_class_budget distill_temperature anchor_weight student_prob_max prob_gap_min <<< "$spec"
  run_job \
    "$label" \
    ...
    SSML_STUDENT_TRUE_PROB_MAX="$student_prob_max" \
    SSML_PEER_STUDENT_PROB_GAP_MIN="$prob_gap_min" \
