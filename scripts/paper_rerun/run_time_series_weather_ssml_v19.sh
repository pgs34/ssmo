...
LOG_DIR="${LOG_DIR:-results/logs/time_series_weather_ssml_v19}"
mkdir -p "$LOG_DIR"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-results/time_series_weather_ssml_rescue_v19}"
...
SSML_ADAPTIVE_DENSE_TOPK_RATIO="${SSML_ADAPTIVE_DENSE_TOPK_RATIO:-0.80}"
SSML_ADAPTIVE_DENSE_TOPK_SCOPE="${SSML_ADAPTIVE_DENSE_TOPK_SCOPE:-positive}"
SSML_ADAPTIVE_DENSE_MAX_SELECTED_RATIO="${SSML_ADAPTIVE_DENSE_MAX_SELECTED_RATIO:-0.30}"
SSML_ADAPTIVE_DENSE_SCORE_SMOOTHING_KERNEL="${SSML_ADAPTIVE_DENSE_SCORE_SMOOTHING_KERNEL:-11}"
SSML_ADAPTIVE_DENSE_WINDOW_EXPAND_KERNEL="${SSML_ADAPTIVE_DENSE_WINDOW_EXPAND_KERNEL:-9}"
SSML_WINDOW_SCORE_KERNEL="${SSML_WINDOW_SCORE_KERNEL:-9}"
CASE_SPECS="${CASE_SPECS:-reswin_t30:0.08:0.0020:0.00:0.30:0.20:3:7:0.30:0.0010 reswin_t50:0.06:0.0015:0.00:0.50:0.25:5:9:0.25:0.0010 reswin_t100:0.05:0.0010:0.00:1.00:0.30:7:11:0.20:0.0005}"
...
echo "[time_series_weather_ssml_v19] output_root=$OUTPUT_ROOT"
echo "[time_series_weather_ssml_v19] gpu=$GPU seeds=$SEEDS"
echo "[time_series_weather_ssml_v19] model_pairs=$MODEL_PAIRS"
echo "[time_series_weather_ssml_v19] case_specs=$CASE_SPECS"

for spec in $CASE_SPECS; do
  IFS=':' read -r label alpha lambda_imitation margin topk_ratio max_selected_ratio smooth_k expand_k residual_beta anchor_weight <<< "$spec"
  run_job \
    "$label" \
