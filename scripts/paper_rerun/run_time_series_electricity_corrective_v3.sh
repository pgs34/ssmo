...
LOG_DIR="${LOG_DIR:-results/logs/time_series_electricity_corrective_v3}"
mkdir -p "$LOG_DIR"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-results/time_series_electricity_corrective_v3}"
SEQ_LEN="${SEQ_LEN:-96}"
...
CASE_SPECS="${CASE_SPECS:-corr_handoff_e12_l15_sp5e4:0.15:0.0005:64:0.00:0:30:60:0.25:12:0:0:0.0 corr_handoff_e14_l15_sp5e4:0.15:0.0005:64:0.00:0:30:60:0.25:14:0:0:0.0}"
...
run_case() {
  ...
  shift
  run_logged_job \
    "time_series_electricity_corrective_v3/$label" \
    "$LOG_DIR/$label.log" \
    env \
...
echo "[time_series_electricity_corrective_v3] output_root=$OUTPUT_ROOT"
echo "[time_series_electricity_corrective_v3] gpu=$GPU seeds=$SEEDS epochs=$EPOCHS"
echo "[time_series_electricity_corrective_v3] init_checkpoint_template=$INIT_CHECKPOINT_TEMPLATE"
echo "[time_series_electricity_corrective_v3] peer_init_checkpoint_template=$PEER_INIT_CHECKPOINT_TEMPLATE"
echo "[time_series_electricity_corrective_v3] case_specs=$CASE_SPECS"

for spec in $CASE_SPECS; do
  IFS=':' read -r label lambda sparsity hidden_dim dropout warmup decay_start decay_end decay_min_scale train_end patience min_epochs min_delta <<< "$spec"
  run_case \
    "$label" \
...
echo "[time_series_electricity_corrective_v3] done"
