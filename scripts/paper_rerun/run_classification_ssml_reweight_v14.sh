...
LOG_DIR="${LOG_DIR:-results/logs/classification_ssml_reweight_v14}"
mkdir -p "$LOG_DIR"
...
SEEDS="${SEEDS:-0 1 2}"
MODEL_PAIRS="${MODEL_PAIRS:-ode_cnn:resnet34_gelu}"
NUM_WORKERS="${NUM_WORKERS:-4}"
BATCH_SIZE="${BATCH_SIZE:-128}"
EPOCHS="${EPOCHS:-100}"
OUTPUT_ROOT="${OUTPUT_ROOT:-results/classification_ssml_reweight_v14}"
DOWNLOAD="${DOWNLOAD:-1}"
...
CASE_SPECS="${CASE_SPECS:-main_pb4_t30:0.30:0.10:0.020:0.02:0.35:4:6.0 main_pb6_t40:0.40:0.10:0.015:0.02:0.40:6:8.0}"
...
run_job() {
  ...
  shift
  run_logged_job \
    "classification_reweight_v14/$label" \
    "$LOG_DIR/$label.log" \
    env \
...
echo "[classification_ssml_reweight_v14] output_root=$OUTPUT_ROOT"
echo "[classification_ssml_reweight_v14] gpu=$GPU datasets=$DATASETS seeds=$SEEDS"
echo "[classification_ssml_reweight_v14] model_pairs=$MODEL_PAIRS"
echo "[classification_ssml_reweight_v14] warmup=$WARMUP_EPOCHS decay=${IMITATION_DECAY_START_EPOCH}->${IMITATION_DECAY_END_EPOCH} min_scale=$IMITATION_DECAY_MIN_SCALE"
echo "[classification_ssml_reweight_v14] freeze_peer=$SSML_FREEZE_PEER student_only=$SSML_STUDENT_ONLY worse_only=$SSML_WORSE_ONLY_UPDATE"
echo "[classification_ssml_reweight_v14] gate=$SSML_GATE_SCORE_MODE transform=$SSML_SCORE_TRANSFORM guidance=$SSML_GUIDANCE_MODE"
echo "[classification_ssml_reweight_v14] peer_correct_only=$SSML_PEER_CORRECT_ONLY student_incorrect_only=$SSML_STUDENT_INCORRECT_ONLY disagreement_only=$SSML_DISAGREEMENT_ONLY class_balanced_topk=$SSML_CLASS_BALANCED_TOPK"
echo "[classification_ssml_reweight_v14] init_checkpoint_template=$INIT_CHECKPOINT_TEMPLATE"
echo "[classification_ssml_reweight_v14] peer_init_checkpoint_template=$PEER_INIT_CHECKPOINT_TEMPLATE"
echo "[classification_ssml_reweight_v14] case_specs=$CASE_SPECS"

for spec in $CASE_SPECS; do
  IFS=':' read -r label topk_ratio alpha lambda_imitation margin prob_threshold per_class_budget distill_temperature <<< "$spec"
  run_job \
    "$label" \
