...
LOG_DIR="${LOG_DIR:-results/logs/time_series_electricity_corrective_v1}"
mkdir -p "$LOG_DIR"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-results/time_series_electricity_corrective_v1}"
SEQ_LEN="${SEQ_LEN:-96}"
...
CASE_SPECS="${CASE_SPECS:-corr_gate32_l15_sp10e4:0.15:0.0010:32:0.00:0:30:60:0.25 corr_gate64_l15_sp5e4:0.15:0.0005:64:0.00:0:30:60:0.25 corr_gate64_do10_l20_sp10e4:0.20:0.0010:64:0.10:0:30:60:0.25 corr_gate32_l10_sp20e4:0.10:0.0020:32:0.00:0:20:50:0.20}"
...
run_case() {
  ...
  shift
  run_logged_job \
    "time_series_electricity_corrective_v1/$label" \
    "$LOG_DIR/$label.log" \
    env \
...
echo "[time_series_electricity_corrective_v1] output_root=$OUTPUT_ROOT"
echo "[time_series_electricity_corrective_v1] gpu=$GPU seeds=$SEEDS"
echo "[time_series_electricity_corrective_v1] model_pairs=$MODEL_PAIRS"
echo "[time_series_electricity_corrective_v1] init_checkpoint_template=$INIT_CHECKPOINT_TEMPLATE"
echo "[time_series_electricity_corrective_v1] peer_init_checkpoint_template=$PEER_INIT_CHECKPOINT_TEMPLATE"
echo "[time_series_electricity_corrective_v1] case_specs=$CASE_SPECS"

for spec in $CASE_SPECS; do
  IFS=':' read -r label lambda sparsity hidden_dim dropout warmup decay_start decay_end decay_min_scale <<< "$spec"
  run_case \
    "$label" \
