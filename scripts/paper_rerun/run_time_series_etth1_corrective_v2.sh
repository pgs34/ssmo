...
LOG_DIR="${LOG_DIR:-results/logs/time_series_etth1_corrective_v2}"
mkdir -p "$LOG_DIR"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-results/time_series_etth1_corrective_v2}"
SEQ_LEN="${SEQ_LEN:-96}"
...
CASE_SPECS="${CASE_SPECS:-biasm4_ramp2to8_lr1e4_l15_sp5e4_h64:0.0001:0.15:0.0005:64:0.00:-4.0:2:8:20:70:0.25 biasm4_ramp2to12_lr1e4_l15_sp5e4_h64:0.0001:0.15:0.0005:64:0.00:-4.0:2:12:20:70:0.25 biasm3_ramp2to8_lr1e4_l15_sp5e4_h64:0.0001:0.15:0.0005:64:0.00:-3.0:2:8:20:70:0.25 biasm4_ramp2to8_lr3e4_l10_sp1e3_h64_do10:0.0003:0.10:0.0010:64:0.10:-4.0:2:8:15:60:0.25}"
...
run_case() {
  ...
  shift
  run_logged_job \
    "time_series_etth1_corrective_v2/$label" \
    "$LOG_DIR/$label.log" \
    env \
...
echo "[time_series_etth1_corrective_v2] output_root=$OUTPUT_ROOT"
echo "[time_series_etth1_corrective_v2] gpu=$GPU seeds=$SEEDS"
echo "[time_series_etth1_corrective_v2] model_pairs=$MODEL_PAIRS"
echo "[time_series_etth1_corrective_v2] init_checkpoint_template=$INIT_CHECKPOINT_TEMPLATE"
echo "[time_series_etth1_corrective_v2] peer_init_checkpoint_template=$PEER_INIT_CHECKPOINT_TEMPLATE"
echo "[time_series_etth1_corrective_v2] case_specs=$CASE_SPECS"

for spec in $CASE_SPECS; do
  IFS=':' read -r label lr lambda sparsity hidden_dim dropout init_bias ramp_start ramp_end decay_start decay_end decay_min_scale <<< "$spec"
  run_case \
    "$label" \
    LR="$lr" \
