...
LOG_DIR="${LOG_DIR:-results/logs/time_series_etth1_corrective_v3}"
mkdir -p "$LOG_DIR"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-results/time_series_etth1_corrective_v3}"
SEQ_LEN="${SEQ_LEN:-96}"
...
CASE_SPECS="${CASE_SPECS:-gateonly8_biasm4_lr1e4_l15_sp5e4_h64:0.0001:0.15:0.0005:64:0.00:-4.0:2:8:8:20:70:0.25 gateonly12_biasm4_lr1e4_l15_sp5e4_h64:0.0001:0.15:0.0005:64:0.00:-4.0:2:12:12:20:70:0.25 gateonly8_biasm3_lr1e4_l15_sp5e4_h64:0.0001:0.15:0.0005:64:0.00:-3.0:2:8:8:20:70:0.25}"
...
run_case() {
  ...
  shift
  run_logged_job \
    "time_series_etth1_corrective_v3/$label" \
    "$LOG_DIR/$label.log" \
    env \
...
echo "[time_series_etth1_corrective_v3] output_root=$OUTPUT_ROOT"
echo "[time_series_etth1_corrective_v3] gpu=$GPU seeds=$SEEDS"
echo "[time_series_etth1_corrective_v3] model_pairs=$MODEL_PAIRS"
echo "[time_series_etth1_corrective_v3] init_checkpoint_template=$INIT_CHECKPOINT_TEMPLATE"
echo "[time_series_etth1_corrective_v3] peer_init_checkpoint_template=$PEER_INIT_CHECKPOINT_TEMPLATE"
echo "[time_series_etth1_corrective_v3] case_specs=$CASE_SPECS"

for spec in $CASE_SPECS; do
  IFS=':' read -r label lr lambda sparsity hidden_dim dropout init_bias ramp_start ramp_end freeze_epochs decay_start decay_end decay_min_scale <<< "$spec"
  run_case \
    "$label" \
    LR="$lr" \
