...
LOG_DIR="${LOG_DIR:-results/logs/time_series_weather_ssml_v14}"
mkdir -p "$LOG_DIR"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-results/time_series_weather_ssml_rescue_v14}"
...
CASE_SPECS="${CASE_SPECS:-main_dense_t100:0.10:0.020:0.00:1.00:7:0.25 main_dense_t50:0.10:0.020:0.00:0.50:7:0.30 main_dense_t100_s9:0.15:0.015:0.00:1.00:9:0.35}"
...
echo "[time_series_weather_ssml_v14] output_root=$OUTPUT_ROOT"
echo "[time_series_weather_ssml_v14] gpu=$GPU seeds=$SEEDS"
echo "[time_series_weather_ssml_v14] model_pairs=$MODEL_PAIRS"
echo "[time_series_weather_ssml_v14] warmup=$WARMUP_EPOCHS decay=${IMITATION_DECAY_START_EPOCH}->${IMITATION_DECAY_END_EPOCH} min_scale=$IMITATION_DECAY_MIN_SCALE"
echo "[time_series_weather_ssml_v14] freeze_peer=$SSML_FREEZE_PEER student_only=$SSML_STUDENT_ONLY worse_only=$SSML_WORSE_ONLY_UPDATE"
echo "[time_series_weather_ssml_v14] gate=$SSML_GATE_SCORE_MODE transform=$SSML_SCORE_TRANSFORM topk_scope=$SSML_TOPK_SCOPE weight_mode=$SSML_SUPERVISED_WEIGHT_MODE guidance=$SSML_GUIDANCE_MODE"
echo "[time_series_weather_ssml_v14] init_checkpoint_template=$INIT_CHECKPOINT_TEMPLATE"
echo "[time_series_weather_ssml_v14] peer_init_checkpoint_template=$PEER_INIT_CHECKPOINT_TEMPLATE"
echo "[time_series_weather_ssml_v14] case_specs=$CASE_SPECS"

for spec in $CASE_SPECS; do
  IFS=':' read -r label alpha lambda_imitation margin topk_ratio smooth_k residual_beta <<< "$spec"
  run_job \
    "$label" \
