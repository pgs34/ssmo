...
LOG_DIR="${LOG_DIR:-results/logs/classification_ssml_reweight_v8}"
mkdir -p "$LOG_DIR"
...
EPOCHS="${EPOCHS:-100}"
OUTPUT_ROOT="${OUTPUT_ROOT:-results/classification_ssml_reweight_v8}"
DOWNLOAD="${DOWNLOAD:-1}"

CIFAR10_TOPKS="${CIFAR10_TOPKS:-0.2 0.3}"
CIFAR100_TOPKS="${CIFAR100_TOPKS:-0.25 0.4}"
...
run_job() {
  ...
  shift
  run_logged_job \
    "classification_reweight_v8/$label" \
    "$LOG_DIR/$label.log" \
    env \
...
echo "[classification_ssml_reweight_v8] output_root=$OUTPUT_ROOT"
echo "[classification_ssml_reweight_v8] gpu=$GPU datasets=$DATASETS seeds=$SEEDS"
echo "[classification_ssml_reweight_v8] cifar10_topks=$CIFAR10_TOPKS"
echo "[classification_ssml_reweight_v8] cifar100_topks=$CIFAR100_TOPKS"
echo "[classification_ssml_reweight_v8] warmup=$WARMUP_EPOCHS decay=${IMITATION_DECAY_START_EPOCH}->${IMITATION_DECAY_END_EPOCH} min_scale=$IMITATION_DECAY_MIN_SCALE"
echo "[classification_ssml_reweight_v8] topk_scope=$SSML_TOPK_SCOPE hotspot_alpha=$SSML_SUPERVISED_HOTSPOT_ALPHA gate=$SSML_GATE_SCORE_MODE transform=$SSML_SCORE_TRANSFORM"
echo "[classification_ssml_reweight_v8] peer_correct_only=$SSML_PEER_CORRECT_ONLY student_incorrect_only=$SSML_STUDENT_INCORRECT_ONLY peer_true_prob_threshold=$SSML_PEER_TRUE_PROB_THRESHOLD"
