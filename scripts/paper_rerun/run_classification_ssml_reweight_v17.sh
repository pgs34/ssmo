...
LOG_DIR="${LOG_DIR:-results/logs/classification_ssml_reweight_v17}"
mkdir -p "$LOG_DIR"
...
EPOCHS="${EPOCHS:-100}"
OUTPUT_ROOT="${OUTPUT_ROOT:-results/classification_ssml_reweight_v17}"
DOWNLOAD="${DOWNLOAD:-1}"
...
SSML_STUDENT_TRUE_PROB_MAX="${SSML_STUDENT_TRUE_PROB_MAX:-0.45}"
...
CASE_SPECS="${CASE_SPECS:-conf_pb20_aw5e4:0.20:0.04:0.010:0.020:0.35:4:6.0:0.0005 conf_pb30_aw1e3:0.30:0.03:0.015:0.030:0.40:5:8.0:0.0010}"
...
run_job() {
  ...
  shift
  run_logged_job \
    "classification_reweight_v17/$label" \
    "$LOG_DIR/$label.log" \
    env \
      ...
      SSML_STUDENT_TRUE_PROB_MAX="$SSML_STUDENT_TRUE_PROB_MAX" \
...
echo "[classification_ssml_reweight_v17] output_root=$OUTPUT_ROOT"
echo "[classification_ssml_reweight_v17] gpu=$GPU datasets=$DATASETS seeds=$SEEDS"
echo "[classification_ssml_reweight_v17] model_pairs=$MODEL_PAIRS"
echo "[classification_ssml_reweight_v17] case_specs=$CASE_SPECS"
