...
LOG_DIR="${LOG_DIR:-results/logs/classification_cifar100_dual_peer_consensus_v1}"
mkdir -p "$LOG_DIR"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-results/classification_cifar100_dual_peer_consensus_v1}"
...
SECONDARY_PEER_BEST_CKPT_TEMPLATE="${SECONDARY_PEER_BEST_CKPT_TEMPLATE:-$BEST_CKPT_TEMPLATE}"

run_case() {
  local label="$1"
  shift
  run_locked_job \
    "classification_cifar100_dual_peer_consensus_v1/$label" \
    "classification_cifar100_dual_peer_consensus_v1/$label" \
    "$LOG_DIR/$label.log" \
    env \
...
echo "[classification_cifar100_dual_peer_consensus_v1] output_root=$OUTPUT_ROOT"
echo "[classification_cifar100_dual_peer_consensus_v1] gpu=$GPU seeds=$SEEDS epochs=$EPOCHS batch_size=$BATCH_SIZE"
echo "[classification_cifar100_dual_peer_consensus_v1] best_ckpt_template=$BEST_CKPT_TEMPLATE"
echo "[classification_cifar100_dual_peer_consensus_v1] peer_best_ckpt_template=$PEER_BEST_CKPT_TEMPLATE"
echo "[classification_cifar100_dual_peer_consensus_v1] secondary_peer_best_ckpt_template=$SECONDARY_PEER_BEST_CKPT_TEMPLATE"
echo "[classification_cifar100_dual_peer_consensus_v1] case_specs=$CASE_SPECS"
...
echo "[classification_cifar100_dual_peer_consensus_v1] done"
