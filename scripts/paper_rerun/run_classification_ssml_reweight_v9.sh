...
LOG_DIR="${LOG_DIR:-results/logs/classification_ssml_reweight_v9}"
mkdir -p "$LOG_DIR"
...
EPOCHS="${EPOCHS:-100}"
OUTPUT_ROOT="${OUTPUT_ROOT:-results/classification_ssml_reweight_v9}"
DOWNLOAD="${DOWNLOAD:-1}"
...
SSML_SUPERVISED_HOTSPOT_ALPHA="${SSML_SUPERVISED_HOTSPOT_ALPHA:-0.25}"
...
run_job() {
  ...
  shift
  run_logged_job \
    "classification_reweight_v9/$label" \
    "$LOG_DIR/$label.log" \
    env \
...
echo "[classification_ssml_reweight_v9] output_root=$OUTPUT_ROOT"
echo "[classification_ssml_reweight_v9] gpu=$GPU datasets=$DATASETS seeds=$SEEDS"
echo "[classification_ssml_reweight_v9] model_pairs=$MODEL_PAIRS"
echo "[classification_ssml_reweight_v9] cifar10_topks=$CIFAR10_TOPKS"
echo "[classification_ssml_reweight_v9] cifar100_topks=$CIFAR100_TOPKS"
echo "[classification_ssml_reweight_v9] warmup=$WARMUP_EPOCHS decay=${IMITATION_DECAY_START_EPOCH}->${IMITATION_DECAY_END_EPOCH} min_scale=$IMITATION_DECAY_MIN_SCALE"
echo "[classification_ssml_reweight_v9] topk_scope=$SSML_TOPK_SCOPE hotspot_alpha=$SSML_SUPERVISED_HOTSPOT_ALPHA weight_mode=$SSML_SUPERVISED_WEIGHT_MODE gate=$SSML_GATE_SCORE_MODE transform=$SSML_SCORE_TRANSFORM"
echo "[classification_ssml_reweight_v9] peer_correct_only=$SSML_PEER_CORRECT_ONLY student_incorrect_only=$SSML_STUDENT_INCORRECT_ONLY peer_true_prob_threshold=$SSML_PEER_TRUE_PROB_THRESHOLD"
