...
def _build_darcy(config: OperatorDataConfig):
    try:
        ...
        return _build_darcy_from_local_pt(config)
    ...
    if config.split_train_ratio < 1.0:
        ...
        val_dataset = dataset.test_loaders[config.darcy_test_resolution].dataset
...
def _build_darcy_from_local_pt(config: OperatorDataConfig):
    root_dir = Path(config.data_root) / "darcy"
    train_path = root_dir / f"darcy_train_{config.darcy_train_resolution}.pt"
    test_path = root_dir / f"darcy_test_{config.darcy_test_resolution}.pt"
    if not train_path.exists() or not test_path.exists():
        raise FileNotFoundError(
            "Darcy fallback requires local PT files under "
            f"'{root_dir}', but missing train='{train_path.exists()}' test='{test_path.exists()}'."
        )

    train_blob = torch.load(train_path, map_location="cpu")
    test_blob = torch.load(test_path, map_location="cpu")
    train_x = train_blob["x"][: int(config.n_train)].unsqueeze(1).to(dtype=torch.float32)
    train_y = train_blob["y"][: int(config.n_train)].unsqueeze(1).to(dtype=torch.float32)
    test_x = test_blob["x"][: int(config.n_test)].unsqueeze(1).to(dtype=torch.float32)
    test_y = test_blob["y"][: int(config.n_test)].unsqueeze(1).to(dtype=torch.float32)
