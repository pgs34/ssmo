...
SSML_SNAPSHOT_ANCHOR_START_EPOCH="${SSML_SNAPSHOT_ANCHOR_START_EPOCH:--1}"
SSML_SNAPSHOT_ANCHOR_WEIGHT="${SSML_SNAPSHOT_ANCHOR_WEIGHT:-0.0}"
SSML_SNAPSHOT_ANCHOR_MASK_MODE="${SSML_SNAPSHOT_ANCHOR_MASK_MODE:-selected}"
SSML_PEER_TAPER_END_EPOCH="${SSML_PEER_TAPER_END_EPOCH:--1}"
SSML_TARGET_ACTIVE_RATIO_START="${SSML_TARGET_ACTIVE_RATIO_START:--1.0}"
SSML_TARGET_ACTIVE_RATIO_END="${SSML_TARGET_ACTIVE_RATIO_END:--1.0}"
SSML_ACTIVE_RATIO_ADAPT_RATE="${SSML_ACTIVE_RATIO_ADAPT_RATE:-0.0}"
...
SSML_EVAL_OUTPUT_MODE="${SSML_EVAL_OUTPUT_MODE:-guided}"
...
SSML_CORRECTION_STUDENT_TRAIN_END_EPOCH="${SSML_CORRECTION_STUDENT_TRAIN_END_EPOCH:--1}"
...
SSML_FUSION_TAIL_START_RATIO="${SSML_FUSION_TAIL_START_RATIO:-0.0}"
SSML_FUSION_MAX_SCALE="${SSML_FUSION_MAX_SCALE:-1.0}"
...
TIME_SERIES_MAX_PARALLEL_RUNS="${TIME_SERIES_MAX_PARALLEL_RUNS:-${MAX_PARALLEL_RUNS:-all}}"
...
echo "[time_series] ssml_eval_output_mode=$SSML_EVAL_OUTPUT_MODE"
...
echo "[time_series] ssml_correction_student_train_end_epoch=$SSML_CORRECTION_STUDENT_TRAIN_END_EPOCH"
...
echo "[time_series] ssml_fusion_tail_start_ratio=$SSML_FUSION_TAIL_START_RATIO"
echo "[time_series] ssml_fusion_max_scale=$SSML_FUSION_MAX_SCALE"
...
echo "[time_series] ssml_snapshot_anchor_start_epoch=$SSML_SNAPSHOT_ANCHOR_START_EPOCH"
echo "[time_series] ssml_snapshot_anchor_weight=$SSML_SNAPSHOT_ANCHOR_WEIGHT"
echo "[time_series] ssml_snapshot_anchor_mask_mode=$SSML_SNAPSHOT_ANCHOR_MASK_MODE"
echo "[time_series] ssml_peer_taper_end_epoch=$SSML_PEER_TAPER_END_EPOCH"
echo "[time_series] ssml_target_active_ratio_start=$SSML_TARGET_ACTIVE_RATIO_START"
echo "[time_series] ssml_target_active_ratio_end=$SSML_TARGET_ACTIVE_RATIO_END"
echo "[time_series] ssml_active_ratio_adapt_rate=$SSML_ACTIVE_RATIO_ADAPT_RATE"
...
echo "[time_series] max_parallel_runs=$TIME_SERIES_MAX_PARALLEL_RUNS"

parallel_exec_init "$TIME_SERIES_MAX_PARALLEL_RUNS"
    ...
    for method in $METHODS; do
        ...
        for MODEL in $INDEPENDENT_MODELS; do
          for seed in $SEEDS; do
            ...
            cmd=(
              ...
              --ssml-snapshot-anchor-start-epoch "$SSML_SNAPSHOT_ANCHOR_START_EPOCH"
              --ssml-snapshot-anchor-weight "$SSML_SNAPSHOT_ANCHOR_WEIGHT"
              --ssml-snapshot-anchor-mask-mode "$SSML_SNAPSHOT_ANCHOR_MASK_MODE"
              --ssml-peer-taper-end-epoch "$SSML_PEER_TAPER_END_EPOCH"
              --ssml-target-active-ratio-start "$SSML_TARGET_ACTIVE_RATIO_START"
              --ssml-target-active-ratio-end "$SSML_TARGET_ACTIVE_RATIO_END"
              --ssml-active-ratio-adapt-rate "$SSML_ACTIVE_RATIO_ADAPT_RATE"
              ...
              --ssml-eval-output-mode "$SSML_EVAL_OUTPUT_MODE"
              ...
              --ssml-correction-student-train-end-epoch "$SSML_CORRECTION_STUDENT_TRAIN_END_EPOCH"
              ...
              --ssml-fusion-tail-start-ratio "$SSML_FUSION_TAIL_START_RATIO"
              --ssml-fusion-max-scale "$SSML_FUSION_MAX_SCALE"
            ...
            job_label="dataset=$dataset model=$MODEL method=$method seed=$seed pred_len=$pred_len"
            echo "[time_series][queue] $job_label"
      ...
      for pair in $MODEL_PAIRS; do
        ...
        for seed in $SEEDS; do
          ...
          job_label="dataset=$dataset pair=$MODEL:$PEER_MODEL method=$method seed=$seed pred_len=$pred_len"
          echo "[time_series][queue] $job_label"
          parallel_exec_submit "$job_label" "${cmd[@]}"
        done
      done
