...
| Operator | Burgers | `FNO x DeepONet` | `0.0000044312` | `FNO independent = 0.0000042316`, `DML = 0.0000093153`, `DeepONet independent = 0.0124357` | strong-model preserve 기준에선 여전히 non-win이다. 새 `student_lift_v2`에서 weak `DeepONet`은 `0.004658`까지 크게 올라왔지만, 이 수치는 strongest `FNO independent`와는 여전히 여러 자릿수 차이가 난다 |
| Operator | Darcy | `DeepONet <- frozen FNO teacher` | `best SSML = 0.013763` | `best control fine-tune = 0.013669`, `original DeepONet independent = 0.018561` | `student_lift_v2`는 약한 `DeepONet`을 `0.018561 -> 0.013763`까지 끌어올렸고 same-campaign control `0.013792`는 소폭 넘겼다. 하지만 best no-peer control across runs `0.013669`는 아직 못 넘어서 weak-model lift claim도 여전히 미완성이다 |
